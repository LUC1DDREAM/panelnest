...
Independent current-API source, ID `multi.luc1d-webtoon`, not WebtoonXYZ (package version 23).
...
- Default popular genre browse, metadata, cursor-paginated Originals episodes, paginated public HTML CANVAS episodes, public reader image URLs, vertical viewer, image Referer. Page descriptions display each image's one-based position from its original reader order.
...
- The Aidoku language selector supports English (`en`), Traditional Chinese (`zh-hant`), Thai (`th`), Indonesian (`id`), Spanish (`es`), French (`fr`), and German (`de`). Search and genre routes use the selected locale; unknown values fall back to English. All three search scopes paginate. CANVAS chapter lists use public paginated series HTML because the official CANVAS episodes API returned HTTP 500 on 2026-09-25; the live episode viewer returned HTTP 200 with 153 image URLs. Optional-page filtering and season-title normalization remain unimplemented.
...
Version 23 maps WEBTOON's official `data-title-unsuitable-for-children` discovery flag and `isMatureTitle` detail state to Aidoku content ratings. Mature titles are marked NSFW; titles explicitly marked suitable are Safe. Missing or unrecognized flags preserve the existing rating.
