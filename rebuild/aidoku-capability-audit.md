...
Audited 2026-09-25 against the pinned Aidoku Rust SDK revision `e1320b0a2e11afb59e4dee374883a2212d325699` in `work/aidoku-rs-inspect/crates/lib/src/structs/source.rs`. The current upstream `main` branch was checked and still points to this commit. The SDK lists the following source traits and the six sources implement them where the site's verified behavior benefits from them.
...
Feature support and test evidence are tracked per source. On 2026-09-25, the user confirmed that Aidoku with IMHentai v31 opened a gallery chapter and displayed its page images. The gallery ID and Aidoku app version were not supplied, so this is evidence for one real reader flow rather than blanket device certification. The other five sources still have no user-confirmed native playback result. The CI suite builds and fixture-tests all six packages but is not a substitute for device testing. WeebCentral requests may receive Cloudflare 403 responses, nhentai may rate-limit requests, and HentaiFox authenticated bookmarks have not been verified with an account session. The official WEBTOON Originals episode API response checked on 2026-09-25 contains episode number, thumbnail, title, viewer URL, exposure timestamp, display-up and BGM fields, but no lock/paid-access indicator; `Chapter.locked` therefore remains unset there.

## Aidoku model-field coverage

The SDK's manga fields (metadata, status, content rating, viewer, update strategy and URLs) and chapter fields (numbering, date, language, scanlator, thumbnail and lock state) were compared against each source parser. WEBTOON and WeebCentral identify vertical-scroll series from their official formats; Asura Scans defaults to vertical for its manhwa/manhua catalog; nhentai derives direction from the official webtoon tag. HentaiFox and IMHentai use right-to-left for standard manga galleries, and IMHentai v31 selects left-to-right from the official Western category. WeebCentral v21 independently extracts chapter and volume numbers from combined title labels. WEBTOON Originals already populate episode dates from the official API. The public CANVAS list also displays date-only episode labels, so source version 22 now parses verified English month-name and ISO formats into UTC-midnight `Chapter.date_uploaded`; unknown or invalid labels remain unset. IMHentai v28 and HentaiFox v32 map their explicit gallery artist metadata to `Manga.artists` rather than misclassifying it as `Manga.authors`; neither site exposes a separate Authors field. nhentai v25 separates the official API's artist and group tags: artists map to `Manga.artists`, groups remain in description metadata, and `Manga.authors` stays empty because the API exposes no author category. Current official Originals episode API responses expose no lock indicator, so WEBTOON `Chapter.locked` cannot be set reliably from that endpoint. Other chapter fields are populated only where the site exposes verified values. No reviewed site exposes a dependable next-update timestamp.

## Current release verification

The current public catalogs list Asura Scans v17, WeebCentral v21, nhentai v25, WEBTOON v23, IMHentai v31 and HentaiFox v35. After commit `c18b5bb`, all eight canonical/experimental JSON feeds across PanelNest and the legacy mirror returned HTTP 200; each of the six packages from every feed matched its source `verification.json` byte count and SHA-256. The source build and Pages deployment passed in Actions run 36133644574. The SDK `main` branch was rechecked and still points to the pinned revision `e1320b0a2e11afb59e4dee374883a2212d325699`.
...
WEBTOON v23 fills the previously unset per-series content rating from the official `data-title-unsuitable-for-children` card flag and `isMatureTitle` series state. The live Tears on a Withered Flower page reported `isMatureTitle: true`; the Space Boy detail fixture reports false, and the genre fixture verifies the mature card flag.
...
- WEBTOON episode lock metadata was checked against a live official Originals episode API response for `title_no=8473`; the response has 53 episodes and no lock, coin, or pass field. `Chapter.locked` cannot currently be set from that response without guessing or changing public access behavior.
